...
import server.generated.MazeCom;
import server.generated.MazeComType;
	...
	public void run() {
		...
		while (failCounter < Settings.LOGINTRIES) {
			...
			if (loginMes != null && loginMes.getMcType() == MazeComType.LOGIN) {
				...
				Debug.print(String.format(server.networking.Messages.getString("LoginThread.successful"), player.getID(),player.getName()), DebugLevel.DEFAULT); //$NON-NLS-1$
		...
		Debug.print(String.format(server.networking.Messages.getString("LoginThread.failed"), player.getID()), DebugLevel.DEFAULT); //$NON-NLS-1$
